...
//
// Scalar reference for saturating_sub. dst = clip(a - b).
...
template <typename T>
inline T saturating_sub_one(T a, T b) {
  if constexpr (sizeof(T) == 8) {
    ...
    long long wide = static_cast<long long>(a) - static_cast<long long>(b);
...
kleidicv_error_t saturating_sub(const T *src_a, size_t src_a_stride,
  ...
  return binary_elementwise<T>(
      ...
      [](T a, T b) { return saturating_sub_one<T>(a, b); });
}

#define INST(T)                                                              \
  template kleidicv_error_t saturating_sub(const T *, size_t, const T *,     \
