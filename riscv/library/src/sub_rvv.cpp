...
//
// RVV 1.0 saturating_sub via vssub / vssubu — single instruction, LMUL=1.
...
#define DEFINE_UNSIGNED(suffix, T, SEW)                                        \
  kleidicv_error_t saturating_sub_##suffix(                                    \
    ...
    return binary_elementwise<T>(                                              \
        ...
        [](auto a, auto b, size_t vl) {                                        \
          return __riscv_vssubu_vv_u##SEW##m1(a, b, vl);                       \
        });                                                                    \
  }
...
#define DEFINE_SIGNED(suffix, T, SEW)                                          \
  kleidicv_error_t saturating_sub_##suffix(                                    \
    ...
    return binary_elementwise<T>(                                              \
        src_a, sa, src_b, sb, dst, sd, w, h,                                   \
        [](auto a, auto b, size_t vl) {                                        \
          return __riscv_vssub_vv_i##SEW##m1(a, b, vl);                        \
        });                                                                    \
  }
