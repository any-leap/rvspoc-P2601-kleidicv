...
inline bool resize_linear_u8_is_implemented(size_t src_width, size_t src_height,
  ...
  // Upsize linear between horizontal ratios of 1.0 and 4.0: a minimal width is
  // needed to execute vector operations
  if (channels <= 3 && dst_width >= src_width && dst_width < 4 * src_width) {
    return (dst_width * channels >= 8 && src_width * channels >= 8);
  }
